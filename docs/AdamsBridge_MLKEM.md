...
3) The FIFO has a maximum depth of 14 entries. When 5 or more entries are occupied, there isn't enough space to accept new inputs while also ensuring that the rejection sampling unit can continue producing outputs in the upcoming cycles. Therefore, the FIFO asserts the FULL flag to prevent accepting additional input from the Keccak core.
