...
if [[ -z ${ADAMSBRIDGE_ROOT:+"empty"} ]]; then
    echo "Must set ADAMSBRIDGE_ROOT prior to running script"
    exit 1
fi
...
exclude_csr='*_csr*.*'
exclude_file='{sglint_waivers,pr_hash,pr_timestamp,.git-comodules,.gitignore,spyglass_lint.policy,ascent.ctl,clp_mapfile,readme.md,README.md,SECURITY.md,CODEOWNERS,c_sample.c}'
