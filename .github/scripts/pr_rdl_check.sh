...
for rdl_file in $(find "${ADAMSBRIDGE_ROOT}/src" -name "*.rdl"); do 
...
if [[ "${rdl_mod_count}" -gt 0 ]]; then
    ...
    bash "${ADAMSBRIDGE_ROOT}/tools/scripts/reg_gen.sh"

    # Check for any file changes
    if [[ $(git status -s --untracked-files=all --ignored=traditional -- "${ADAMSBRIDGE_ROOT}/src/" | wc -l) -gt 0 ]]; then
