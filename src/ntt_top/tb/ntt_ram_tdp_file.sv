...
module ntt_ram_tdp_file 
    ...
    logic [ABR_MEM_DATA_WIDTH-1:0] data_tb, masked_data_tb;
    ...
    begin : writing_memory
        if (!reset_n) begin
            ...
            for (int i0 = 0; i0 < MLDSA_N/COEFF_PER_CLK; i0++) begin
                if (mlkem) begin
                    for (int k = 0; k < ABR_MEM_DATA_WIDTH/24; k++) begin
            ...
            for (int i0 = 'hC0; i0 < 'hC0+MLDSA_N/COEFF_PER_CLK; i0++) begin //Choosing 'hC0 as base addr to load masked values
                if (mlkem) begin
                    ...
                    rand_input = {24'($urandom() % MLKEM_Q),
                                  ...
                                  24'($urandom() % MLKEM_Q),
                                  24'($urandom() % MLKEM_Q)};
                    for (int k = 0; k < 4/*ABR_MEM_DATA_WIDTH/24*/; k++) begin
                ...
                mem[i0] <= ABR_MEM_DATA_WIDTH'(rand_input); //mem_tb[i0];
                mem[i0+'hC0] <= ABR_MEM_DATA_WIDTH'(masked_data_tb); //ABR_MEM_DATA_WIDTH'((mem_tb[i0-'hc0] - data_tb) % MLDSA_Q); //Storing the second share at offset 'hC0
