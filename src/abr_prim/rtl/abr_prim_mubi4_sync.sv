...
`ifdef ABR_INC_ASSERT
      ...
      `ABR_ASSERT(OutputIfUnstable_A, sig_unstable |-> mubi_o == {NumCopies{reset_value}})
      `ABR_ASSERT(OutputDelay_A,
              rst_b |-> ##[3:4] sig_unstable || mubi_o == {NumCopies{$past(mubi_in_sva_q, 2)}})
`endif
...
`ifdef ABR_INC_ASSERT
      ...
      `ABR_ASSERT(OutputDelay_A,
              rst_b |-> ##3 (mubi_o == {NumCopies{$past(mubi_in_sva_q, 2)}} ||
...
`endif
  ...
  `ABR_ASSERT_KNOWN(OutputsKnown_A, mubi_o)

endmodule : abr_prim_mubi4_sync
