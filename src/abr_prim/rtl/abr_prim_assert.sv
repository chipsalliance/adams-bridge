...
// Defining ABR_SVA marks these as the canonical core definitions so the
// tool-flavored fallback blocks below (guarded by `ifndef ABR_SVA) are skipped,
...
`ifdef ABR_ASSERT_ON                                                           \
  assert_name: assert property (@(posedge clk) disable iff ((rst !== 0) | (en === 0)) (prop))    \
        ...
        `ABR_ASSERT_RPT(`ABR_STRINGIFY(assert_name))                           \
    end                                                                        \
`endif
