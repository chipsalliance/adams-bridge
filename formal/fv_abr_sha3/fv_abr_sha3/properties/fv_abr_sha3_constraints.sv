...
module fv_abr_sha3_constraints
    import abr_sha3_pkg::*;
...
#(
    ...
    //process_i cannot be set to true unless the state is StAbsorb
    assume_process_i_low: assume property(disable iff(!pi_rst_b)
        po_sha3_fsm_o != StAbsorb
    |->
        !pi_process_i
    );
    ...
    assume_no_several_starts: assume property(disable iff(!pi_rst_b)
        pi_msg_start_i
    |->
        ##1 (!pi_msg_start_i) until (po_absorbed_o == abr_prim_mubi_pkg::MuBi4True)
    );
    ...
    // start_i is a pulse
    assume_start_i_pulse: assume property(disable iff(!pi_rst_b)
        pi_start_i
    |->
        ##1 !pi_start_i
    );
    ...
    assume_msg_start_i_pulse: assume property(disable iff(!pi_rst_b)
        pi_msg_start_i
    |->
        ##1 !pi_msg_start_i
    );

    // msg_start_i is set immediately after start_i
    assume_msg_start_i_after_start_i: assume property(disable iff(!pi_rst_b)
        pi_start_i
    |->
        ##1 pi_msg_start_i
    );
    ...
    //         fv_run_i_ff <= 1'b0;
    //     end else if(pi_run_i) begin
    //             fv_run_i_ff <= 1'b1;
    //     end
    // end

    // If state_valid_o and state_valid_hold_i are high, then run_i cannot go high
    assume_run_i_during_hold: assume property(disable iff(!pi_rst_b)
        po_state_valid_o &&
...
bind abr_sha3 fv_abr_sha3_constraints fv_abr_sha3_constraints_i (
    .pi_clk_i(clk_i),
    .pi_rst_b(rst_b),
    .pi_zeroize(zeroize),
