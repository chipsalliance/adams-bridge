...
module fv_ntt_ctrl_pwm_intt_m(
  ...
  input st_NttPwBaseAddrs pwo_mem_base_addr_in,
  ...
  input logic unsigned [43:0] buf_wrptr_reg,
  input logic unsigned [3:0] chunk_count,
  input logic unsigned [79:0] chunk_count_reg,
  ...
  input e_PwmInttReadStatesType read_state,
  ...
  input e_PwmInttWriteStatesType write_state,
...
assign getTwiddleRandOffsets_0_i = getTwiddleRandOffsets(chunk_count_reg, buf_wrptr_reg);
...
assign getTwiddleAddrReg_0_i = getTwiddleAddrReg(incr_twiddle_addr_reg, ((read_state == PwmInttReadIdle) || ((read_state == PwmInttReadStage) && !butterfly_ready_in)), getTwiddleIncrAddr_0_i, twiddle_addr_reg);

logic unsigned [79:0] getChunkCountReg_0_i;
assign getChunkCountReg_0_i = getChunkCountReg(read_state, butterfly_ready_in, rounds_count, chunk_count, chunk_count_reg);

logic unsigned [1:0] getMemRdIndexOfst_0_i;
assign getMemRdIndexOfst_0_i = getMemRdIndexOfst(index_count, index_rand_offset);

logic unsigned [14:0] getPwMemRdAddrNxt_0_i;
assign getPwMemRdAddrNxt_0_i = getPwMemRdAddrNxt(((rounds_count == 3'd0) ? pwo_mem_base_addr_in.pw_base_addr_a : 15'd0), chunk_count, 15'd1, getMemRdIndexOfst_0_i);

logic unsigned [14:0] getPwMemRdAddrNxt_1_i;
assign getPwMemRdAddrNxt_1_i = getPwMemRdAddrNxt(((rounds_count == 3'd0) ? pwo_mem_base_addr_in.pw_base_addr_b : 15'd0), chunk_count, 15'd1, getMemRdIndexOfst_0_i);

logic unsigned [14:0] getPwMemRdAddrNxt_2_i;
assign getPwMemRdAddrNxt_2_i = getPwMemRdAddrNxt(15'd0, chunk_count, 15'd1, getMemRdIndexOfst_0_i);

logic unsigned [14:0] getPwMemRdRstAddr_0_i;
assign getPwMemRdRstAddr_0_i = getPwMemRdRstAddr(shuffle_en_in, chunk_rand_offset, ((rounds_count == 3'd0) ? pwo_mem_base_addr_in.pw_base_addr_a : 15'd0));

logic unsigned [14:0] getPwMemRdIncrAddrAb_0_i;
assign getPwMemRdIncrAddrAb_0_i = getPwMemRdIncrAddrAb(shuffle_en_in, getPwMemRdAddrNxt_0_i, pw_mem_rd_addr_a_reg, 15'd1);

logic unsigned [14:0] getPwMemRdAddrAb_0_i;
assign getPwMemRdAddrAb_0_i = getPwMemRdAddrAb((write_state == PwmInttWriteIdle), (((read_state == PwmInttReadExec) && (rounds_count == 3'd0)) && sampler_valid_in), getPwMemRdRstAddr_0_i, getPwMemRdIncrAddrAb_0_i, pw_mem_rd_addr_a_reg);

logic unsigned [14:0] getPwMemRdRstAddr_1_i;
assign getPwMemRdRstAddr_1_i = getPwMemRdRstAddr(shuffle_en_in, chunk_rand_offset, ((rounds_count == 3'd0) ? pwo_mem_base_addr_in.pw_base_addr_b : 15'd0));

logic unsigned [14:0] getPwMemRdIncrAddrAb_1_i;
assign getPwMemRdIncrAddrAb_1_i = getPwMemRdIncrAddrAb(shuffle_en_in, getPwMemRdAddrNxt_1_i, pw_mem_rd_addr_b_reg, 15'd1);

logic unsigned [14:0] getPwMemRdAddrAb_1_i;
assign getPwMemRdAddrAb_1_i = getPwMemRdAddrAb((write_state == PwmInttWriteIdle), (((read_state == PwmInttReadExec) && (rounds_count == 3'd0)) && sampler_valid_in), getPwMemRdRstAddr_1_i, getPwMemRdIncrAddrAb_1_i, pw_mem_rd_addr_b_reg);

logic unsigned [14:0] getPwMemRdRstAddr_2_i;
assign getPwMemRdRstAddr_2_i = getPwMemRdRstAddr(shuffle_en_in, chunk_rand_offset, 15'd0);

logic unsigned [14:0] getPwMemRdShuffledIncrAddrC_0_i;
assign getPwMemRdShuffledIncrAddrC_0_i = getPwMemRdShuffledIncrAddrC(accumulate_in, getPwMemRdAddrNxt_2_i);

logic unsigned [14:0] getPwMemRdUnshuffledIncrAddrC_0_i;
assign getPwMemRdUnshuffledIncrAddrC_0_i = getPwMemRdUnshuffledIncrAddrC(accumulate_in, pw_mem_rd_addr_c_reg, 15'd1);

logic unsigned [14:0] getPwMemRdAddrC_0_i;
assign getPwMemRdAddrC_0_i = getPwMemRdAddrC((write_state == PwmInttWriteIdle), (((read_state == PwmInttReadExec) && (rounds_count == 3'd0)) && sampler_valid_in), shuffle_en_in, getPwMemRdRstAddr_2_i, getPwMemRdShuffledIncrAddrC_0_i, getPwMemRdUnshuffledIncrAddrC_0_i, pw_mem_rd_addr_c_reg);

logic unsigned [14:0] getMemRdBaseAddr_0_i;
assign getMemRdBaseAddr_0_i = getMemRdBaseAddr(rounds_count, ntt_mem_base_addr_in);

logic unsigned [43:0] getBufWrptrReg_0_i;
assign getBufWrptrReg_0_i = getBufWrptrReg(read_state, butterfly_ready_in, rounds_count, getMemRdIndexOfst_0_i, buf_wrptr_reg);
...
assign getIndexCount_0_i = getIndexCount(read_state, rounds_count, sampler_valid_in, index_count);
...
assign getBufWrptr_0_i = getBufWrptr(shuffle_en_in, 0, buf_wrptr, buf_wrptr_reg);
...
assign getBufWrptr_1_i = getBufWrptr(shuffle_en_in, butterfly_ready_in, buf_wrptr, buf_wrptr_reg);
...
assign getBufWrptr_2_i = getBufWrptr(shuffle_en_in, 1, buf_wrptr, buf_wrptr_reg);

logic unsigned [1:0] getBufWrptr_3_i;
assign getBufWrptr_3_i = getBufWrptr(shuffle_en_in, !shuffle_en_in, buf_wrptr, buf_wrptr_reg);

logic unsigned [2:0] getRoundsCount_1_i;
assign getRoundsCount_1_i = getRoundsCount(rst_rounds_in, (({ 61'h0, rounds_count} ) < 64'd4), rounds_count);
...
|->
  ...
  masking_en_ctrl_out == ($past(rounds_count, 1) == 3'd0) &&
  opcode_out == unsigned'(3'((($past(rounds_count, 1) == 3'd0) ? 5 : 1))) &&
  pw_wren_out == 0;
endproperty
...
|->
  ##1
  read_exec &&
  bf_enable_reg == ($past(read_state, 1) == PwmInttReadExec) &&
  bf_enable_reg_d2 == $past(bf_enable_reg, 1) &&
  buf_wrptr_reg == $past(getBufWrptrReg_0_i, 1) &&
  chunk_count_reg == $past(getChunkCountReg_0_i, 1) &&
  incr_twiddle_addr_reg == ($past(read_state, 1) == PwmInttReadExec) &&
  index_count == $past(getIndexCount_0_i, 1) &&
  index_rand_offset == ((($past(index_count, 1) == 2'd3) || ((($past(read_state, 1) == PwmInttReadStage) && ($past(write_state, 1) == PwmInttWriteStage)) && ($past(rounds_count, 1) != 3'd4))) ? { $past(random_in, 1) }[1:0] : $past(index_rand_offset, 1)) &&
  mem_rd_addr == (($past(rounds_count, 1) == 3'd0) ? $past(mem_rd_addr, 1) : $past(wraparoundMemAddr_1_i, 1)) &&
  mem_rd_addr_out == (($past(rounds_count, 1) == 3'd0) ? $past(mem_rd_addr, 1) : $past(wraparoundMemAddr_1_i, 1)) &&
  mem_rd_en_reg == ((($past(read_state, 1) == PwmInttReadExec) && (({ 61'd0, $past(rounds_count, 1)} ) > 64'd0)) && (({ 49'd0, $past(mem_rd_addr, 1)} ) <= (64'd63 + ({ 49'd0, $past(getMemRdBaseAddr_0_i, 1)} )))) &&
  pw_mem_rd_addr_a_out == $past(getPwMemRdAddrAb_0_i, 1) &&
  pw_mem_rd_addr_a_reg == $past(getPwMemRdAddrAb_0_i, 1) &&
  pw_mem_rd_addr_b_out == $past(getPwMemRdAddrAb_1_i, 1) &&
  pw_mem_rd_addr_b_reg == $past(getPwMemRdAddrAb_1_i, 1) &&
  pw_mem_rd_addr_c_out == $past(getPwMemRdAddrC_0_i, 1) &&
  pw_mem_rd_addr_c_reg == $past(getPwMemRdAddrC_0_i, 1) &&
  pw_rden_reg == ((($past(read_state, 1) == PwmInttReadExec) && ($past(rounds_count, 1) == 3'd0)) && $past(sampler_valid_in, 1)) &&
  rd_valid_count == $past(getRdValidCount_0_i, 1) &&
  read_state == PwmInttReadExec &&
  twiddle_addr_reg == $past(getTwiddleAddrReg_0_i, 1) &&
  twiddle_addr_reg_d2 == ($past(shuffle_en_in, 1) ? ($past(getTwiddleRandOffsets_0_i[rounds_count], 1) + $past(twiddle_offset_array_0_i[rounds_count], 1)) : ($past(twiddle_addr_reg, 1) + $past(twiddle_offset_array_0_i[rounds_count], 1))) &&
...
|->
  ...
  index_count == $past(getIndexCount_0_i, 1) &&
  index_rand_offset == ((($past(index_count, 1) == 2'd3) || ((($past(read_state, 1) == PwmInttReadStage) && ($past(write_state, 1) == PwmInttWriteStage)) && ($past(rounds_count, 1) != 3'd4))) ? { $past(random_in, 1) }[1:0] : $past(index_rand_offset, 1)) &&
  mem_rd_addr == (($past(shuffle_en_in, 1) && (({ 61'd0, $past(rounds_count, 1)} ) > 64'd0)) ? 15'((({ 49'd0, $past(getMemRdBaseAddr_0_i, 1)} ) + (64'd4 * ({ 60'd0, $past(chunk_rand_offset, 1)} )))) : ({ 49'd0, $past(getMemRdBaseAddr_0_i, 1)} )) &&
  mem_rd_addr_out == (($past(shuffle_en_in, 1) && (({ 61'd0, $past(rounds_count, 1)} ) > 64'd0)) ? 15'((({ 49'd0, $past(getMemRdBaseAddr_0_i, 1)} ) + (64'd4 * ({ 60'd0, $past(chunk_rand_offset, 1)} )))) : ({ 49'd0, $past(getMemRdBaseAddr_0_i, 1)} )) &&
  mem_rd_en_reg == ((($past(read_state, 1) == PwmInttReadExec) && (({ 61'd0, $past(rounds_count, 1)} ) > 64'd0)) && (({ 49'd0, $past(mem_rd_addr, 1)} ) <= (64'd63 + ({ 49'd0, $past(getMemRdBaseAddr_0_i, 1)} )))) &&
  pw_mem_rd_addr_a_out == $past(getPwMemRdAddrAb_0_i, 1) &&
  pw_mem_rd_addr_a_reg == $past(getPwMemRdAddrAb_0_i, 1) &&
  pw_mem_rd_addr_b_out == $past(getPwMemRdAddrAb_1_i, 1) &&
