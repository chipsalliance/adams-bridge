...
module fv_ntt_ctrl_pwm_intt_additional_properties
...
(
  ...
  property assert_bf_enable_p;
    po_bf_enable == (pi_shuffle_en ? ntt_ctrl.bf_enable_reg_d2 : ntt_ctrl.bf_enable_reg);
  endproperty
  ...
  logic [14:0] mem_rd_base_addr;
  assign mem_rd_base_addr = ntt_ctrl.rounds_count == 0 ? 0 : (ntt_ctrl.rounds_count[0] ? pi_ntt_mem_base_addr.interim_base_addr : pi_ntt_mem_base_addr.dest_base_addr);

  property assert_mem_rd_en_p;
    po_mem_rd_en == (pi_shuffle_en ? ntt_ctrl.mem_rd_en_reg : (ntt_ctrl.read_fsm_state_ps == RD_EXEC && ntt_ctrl.rounds_count > 0 && po_mem_rd_addr <= (mem_rd_base_addr + MEM_LAST_ADDR)));
  endproperty
  ...
  property assert_buf_wr_rst_count_p;
    po_buf_wr_rst_count == (ntt_ctrl.read_fsm_state_ps == RD_IDLE || (ntt_ctrl.write_fsm_state_ps == WR_STAGE && ntt_ctrl.rounds_count > 0));
  endproperty
  ...
  property assert_buf_rd_rst_count_p;
    po_buf_rd_rst_count == (ntt_ctrl.read_fsm_state_ps == RD_IDLE || (ntt_ctrl.write_fsm_state_ps == WR_STAGE && ntt_ctrl.rounds_count > 0));
  endproperty
  ...
  property assert_pw_rden_p;
    po_pw_rden == (pi_shuffle_en ? ntt_ctrl.pw_rden_reg : (ntt_ctrl.read_fsm_state_ps == RD_EXEC && ntt_ctrl.rounds_count == 0 && pi_sampler_valid));
  endproperty
...
bind ntt_ctrl fv_ntt_ctrl_pwm_intt_additional_properties ntt_ctrl_pwm_intt_additional_inst(
    .pi_clk (clk),
    .pi_reset_n (reset_n),
    .pi_zeroize (zeroize),
