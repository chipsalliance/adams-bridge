...
module fv_sigdecode_z_top_constraints
...
#(
    ...
    always_ff @(posedge pi_clk) begin
        ...
        end else if(!ongoing_computation || po_sigdecode_z_done) begin
            ongoing_computation <= pi_sigdecode_z_enable;
        end
    end

    // Set enable only when it is not busy
    assume_only_enable_when_idle : assume property (
        pi_sigdecode_z_enable
    |->
        !ongoing_computation || po_sigdecode_z_done
    );

endmodule

bind sigdecode_z_top fv_sigdecode_z_top_constraints #(
    .MEM_ADDR_WIDTH (MEM_ADDR_WIDTH),
    .REG_SIZE       (REG_SIZE      ),
    .GAMMA1         (GAMMA1        )
 ) fv_sigdecode_z_constraints_i (    
    //#$bind
    .pi_clk (clk),
    .pi_reset_n (reset_n),
    .pi_zeroize (zeroize),
