...
SC_MODULE(ntt_ctrl_ct_mode) {
public:
  SC_CTOR(ntt_ctrl_ct_mode) {
    SC_THREAD(read)
    SC_THREAD(write)
  }
  ...
  [[noreturn]] void write() {
    ...
    while (true) {
      if (write_state == CtWriteStatesType::CtWriteIdle) {
       ...
       chunk_count_reg_port     -> get(chunk_count_reg);
       buf_rdptr_reg_port       -> get(buf_rdptr_reg);
       mem_wr_next_addr         = shuffle_en ? static_cast<sc_uint<16>>((4 * chunk_count_reg) + ( buf_rdptr_reg) + base_address_temp_write)
