...
#(
    ...
    endproperty
    assert_buf_enable_a: assert property (disable iff(!pi_reset_n || pi_zeroize) buf_enable_p);
  ...
  assign mem_rd_en_1 = (pi_shuffle_en)? (((po_mem_rd_addr <= 63 + ntt_ctrl.mem_rd_base_addr) && !((ntt_ctrl.buf_count == 3) && (ntt_ctrl.rd_valid_count == 60)) ) ? 1 : 0)
                   : ((po_mem_rd_addr <= ntt_ctrl.mem_rd_base_addr + 63)) ? 1 : 0;
                    
  assign mem_rd_en_2 = ((ntt_ctrl.read_fsm_state_ps == RD_STAGE) || ((ntt_ctrl.read_fsm_state_ps == EXEC_WAIT)|| (ntt_ctrl.read_fsm_state_ps == RD_IDLE)))? 0 : ((ntt_ctrl.read_fsm_state_ps == RD_BUF) || (ntt_ctrl.read_fsm_state_ps == RD_EXEC) )&& mem_rd_en_1;
  ...
  endproperty  
  mem_rd_en_p: assert property(disable iff(!pi_reset_n || pi_zeroize) mem_rd_en_a);
  ...
  endproperty  
  not_same_address_a : assert property (disable iff(!pi_reset_n || pi_zeroize)not_same_address_p);
