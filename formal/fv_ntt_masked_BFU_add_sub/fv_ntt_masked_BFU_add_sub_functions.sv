...
package fv_ntt_masked_BFU_add_sub_functions;


function logic unsigned [1:0][45:0] add_sub(logic unsigned [1:0][45:0] u, logic unsigned [1:0][45:0] v, logic unsigned [0:0] sub);
  logic unsigned [1:0][45:0] temp; // @ ntt_masked_BFU_add_sub.h:15:5
  logic signed [31:0] i;           // @ ntt_masked_BFU_add_sub.h:17:10

  temp = '{ 0: 46'd0, 1: 46'd0 };

  for (i = 32'd0; i < 32'd2; i = i + 32'd1) begin
    if (!(sub != 64'd0)) begin
      temp[unsigned'(i)] = u[unsigned'(i)] + v[unsigned'(i)];
    end else begin
      temp[64'd0] = (u[64'd0] + 64'd8380417) - v[64'd0];
      temp[64'd1] = (u[64'd1] + (~v[64'd1])) + 64'd1;
    end
  end
...
function logic unsigned [45:0][1:0] transpose(logic unsigned [1:0][45:0] temp);
  logic unsigned [45:0][1:0] result; // @ ntt_masked_BFU_add_sub.h:31:5
  logic signed [31:0] i;             // @ ntt_masked_BFU_add_sub.h:33:10
...
function logic unsigned [1:0][45:0] unpacked(logic unsigned [45:0][1:0] temp);
