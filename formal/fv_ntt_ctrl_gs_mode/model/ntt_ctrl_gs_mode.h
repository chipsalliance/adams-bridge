...
sc_uint<2> getBufWrptr(bool shuffle_en, bool buf_wren_intt, sc_uint<2> buf_wrptr, sc_uint<26> buf_wrptr_reg) {
  if (buf_wren_intt) {
    if (shuffle_en) {
...
SC_MODULE(ntt_ctrl_gs_mode) {
public:
  SC_CTOR(ntt_ctrl_gs_mode) {
    SC_THREAD(read)
    SC_THREAD(write)
  }
